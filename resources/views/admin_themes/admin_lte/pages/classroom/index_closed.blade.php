...
@section('page_title', trans('pages.admin_closed_classrooms_title'))
@section('page_description', trans('pages.admin_closed_classrooms_desc'))
@section('page_breadcrumb')
    <ol class="breadcrumb">
        <li><a href="{{ adminUrl() }}"><i class="fa fa-home"></i> {{ trans('pages.admin_dashboard_title') }}</a></li>
        <li><a href="{{ adminUrl('closed-classrooms') }}">{{ trans('pages.admin_closed_classrooms_title') }}</a></li>
    </ol>
@endsection
@section('lib_styles')
                        ...
                        <table class="table table-bordered">
                            <thead>
                                <tr>
                                    ...
                                    <th>{{ trans('label.hours_salary') }} ({{ $salary_jump_currency }} / 1 {{ trans_choice('label.hour_lc', 1) }})</th>
                                    <th>{{ trans('label.created_at')}}</th>
                                    <th>{{ trans('label.status_closed')}}</th>
                                    @if($canDoAction)
                            ...
                            <tbody>
                                @foreach($classrooms as $classroom)
                                    <tr>
                                        ...
                                        <td>{{ $classroom->id }}::{{ $classroom->name }}</td>
                                        ...
                                        <td>{{ number_format($classroom->hours_salary) }}</td>
                                        <td>{{ $classroom->created_at }}</td>
                                        <td>{{ $classroom->closed_at }}</td>
                                        @if($canDoAction)
                                            <td>
                                                <a href="{{ adminUrl('classrooms/{id}/edit', ['id'=> $classroom->id]) }}">
                                                    {{ trans('form.action_edit') }}
                                                </a>
                                                <a class="classroom-open" href="{{ addRdrUrl(adminUrl('classrooms/{id}', ['id'=> $classroom->id]) . '?open=1') }}">
